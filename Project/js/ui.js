...
export const notify = (msg, type = 'success') => {
    ...
    contentEl.classList.remove('notify-success', 'notify-error');
    contentEl.classList.add(type === 'success' ? 'notify-success' : 'notify-error');
...
export const resetWorkspace = () => {
    ...
    // --- UPDATED: HIDE HEADER ACTIONS & RESET HEADER TEXT ---
    $('header-toolbar').classList.add('hidden');
    $('header-project-name').textContent = "Select Project";
    // -----------------------------------------------------
...
export const updateUIForLoadedProject = (projectData) => {
    ...
    // --- UPDATED: SHOW HEADER ACTIONS ---
    $('header-toolbar').classList.remove('hidden');
    // ------------------------------------

    $('save-template-name-input').value = projectData.name;
    
    // --- UPDATED: RENDER HEADER DEPLOY BUTTONS ---
    const deployContainer = $('header-deploy-container');
    const needsUpdate = projectData.isDirty && projectData.deploymentUrl;
    
    let html = '';
    if (projectData.deploymentUrl) {
        html += `
            <a href="${projectData.deploymentUrl}" target="_blank" class="btn-header-deploy success" title="Visit Live Site"><i class="fas fa-external-link-alt"></i> Visit</a>
            <button class="btn-header-deploy secondary ${needsUpdate ? 'needs-update' : ''}" id="header-redeploy-btn" title="Re-deploy to update"><i class="fas fa-sync-alt"></i> Redeploy</button>
        `;
    } else {
        html += `<button class="btn-header-deploy primary" id="header-deploy-btn" title="Deploy to Netlify"><i class="fas fa-rocket"></i> Deploy</button>`;
    }
    if(deployContainer) deployContainer.innerHTML = html;
    // ---------------------------------------------
};
...
export const showCode = () => {
    ...
    if (lineNumbers) {
        ...
        if (window.setEditorValue) {
            window.setEditorValue('html', html);
            window.setEditorValue('css', css);
            window.setEditorValue('js', js);
        } else {
...
export const toggleCodeEditorReadOnly = (isReadOnly) => {
    ...
    // old textarea-based behavior
    document.querySelectorAll('.code-editor').forEach(editor => {
        editor.readOnly = isReadOnly;
        editor.classList.toggle('read-only', isReadOnly);
    });
    // New: CodeMirror-aware behavior (set option for each editor)
    if (window.editors) {
        ['html','css','js'].forEach(lang => {
            const ed = window.editors[lang];
            if (!ed) return;
            ed.setOption('readOnly', isReadOnly ? 'nocursor' : false);
            // Visual cue
            const wrapper = ed.getWrapperElement();
            wrapper.classList.toggle('cm-readonly', isReadOnly);
        });
    }
